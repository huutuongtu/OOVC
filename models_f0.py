...
class SynthesizerTrn(nn.Module):
  ...
  def forward(self, c, spec, g=None, f0=None, mel=None, c_lengths=None, spec_lengths=None):
    ...
    f0 = torch.bucketize(f0, self.f0_bins.to(f0.device)).to(f0.device).squeeze(1) # B x T
    f0 = self.f0_emb(f0).transpose(1,2) # B x H x T
    f0 = self.f0_conv(f0) # B x H x T
    ...
    o = self.dec(z_slice + f0_slice, g=g) #decode with f0 condition
    
    return o, ids_slice, spec_mask, (z, z_p, m_p, logs_p, m_q, logs_q)
      
  def get_content(self, c, g=None, mel=None, c_lengths=None):
    if c_lengths == None:
      c_lengths = (torch.ones(c.size(0)) * c.size(-1)).to(c.device)
